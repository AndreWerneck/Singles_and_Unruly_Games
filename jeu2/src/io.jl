...
function displaySolution(t::Array{Int64,2},y::Union{Array{Int64,2},Array{VariableRef,2}})

    n = size(t,1)

    if (isa(y,Matrix{VariableRef}))

        for i in 1:n
            print("\n")
            for j in 1:n
                if value(v[i, j]) == 0
                    print(round(Int, value(t[i, j])), " ")
                end
                if value(v[i, j]) != 0
                    print("■ ")
                end
...
- t: 2-dimensional array of size n*n
"""
function writeSolution(fout::IOStream, t::Array{Int,2})
